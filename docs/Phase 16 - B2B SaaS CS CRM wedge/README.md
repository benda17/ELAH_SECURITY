...
| 16 | Surface CRM analytics in Banking System dashboard | `task-16-surface-crm-analytics-in-banking-system-dashboar` | in_review | — | Founder platform `app/banking/crm/*`. Reads CRM DB only. Do not merge CRM rows into banking Neon. |
| 17 | Seed CRM demo users and command logs | `task-16-seed-crm-demo-users-and-command-logs` | in_progress | — | GitHub `prisma/seed.ts` + `scripts/seed-crm-traffic.ts`. Identities on hosted Neon; 10k traffic pack local unless re-run hosted. Synthetic/simulator only. |
| 18 | Point founder dashboard at live CRM Vercel URL | `task-16-point-founder-dashboard-at-live-crm-vercel-url` | **in_progress** | — | Hosted CRM is `https://elahcrmsystem.vercel.app`. Local `.env` can use that or `:3003`. Production founder Vercel env still needs the founder to paste + redeploy if unset. |
